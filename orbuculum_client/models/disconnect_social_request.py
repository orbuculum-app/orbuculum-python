...
class DisconnectSocialRequest(BaseModel):
    ...
    """ # noqa: E501
    provider: StrictStr = Field(description="Social auth provider to disconnect")
    __properties: ClassVar[List[str]] = ["provider"]

    @field_validator('provider')
    def provider_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['google', 'facebook', 'linkedin']):
            raise ValueError("must be one of enum values ('google', 'facebook', 'linkedin')")
        return value
